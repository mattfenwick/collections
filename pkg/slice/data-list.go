...
func Any[A any](f F1[A, bool], xs []A) bool {
	return Foldl(builtin.Or, false, Map(f, xs))
}
...
func Product[A builtin.Number](xs []A) A {
	return Foldl[A, A](builtin.Times[A], 1, xs)
}
...
func Replicate[A any](count int, a A) []A {
	return Iterate(count, function.Id[A], a)
}
...
func Break[A any](pred F1[A, bool], xs []A) *Pair[[]A, []A] {
	...
	var f F1[A, bool] = function.Compose(builtin.Not, pred)
	var g F1[F1[A, bool], F1[[]A, *Pair[[]A, []A]]] = function.Partial2(Span[A])
...
func GroupConsecutiveBy[A any](g F2[A, A, bool], xs []A) [][]A {
	...
	for {
		...
		pair := Span[A](function.Partial2(g)(rest[0]), rest[1:])
