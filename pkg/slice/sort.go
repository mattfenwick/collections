...
func SortOnOrd[A any, B Ord[B]](on func(A) B, xs []A) []A {
	return SortOnBy(on, Compare[B], xs)
}
...
func SortOnBy[A any, B any](on func(A) B, compare Comparator[B], xs []A) []A {
	pairs := Map(func(a A) *Pair[A, B] { return NewPair(a, on(a)) }, xs)
